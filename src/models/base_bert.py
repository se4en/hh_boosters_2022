...
class BertClassifier(nn.Module):
    def __init__(
        self, bert_path: str, head: nn.Module, bert_layers_to_freeze: int = 12
    ):
        super().__init__()
    ...
    def forward(
        ...
        pos_outputs = self._bert(
            pos_tokens, token_type_ids=None, attention_mask=pos_mask
        )[0]
        neg_outputs = self._bert(
            neg_tokens, token_type_ids=None, attention_mask=neg_mask
        )[0]
...
class LstmAttention(nn.Module):
    def __init__(
        ...
        batch_size: int,
        pos_lstm: nn.Module,
        neg_lstm: Optional[nn.Module] = None,
        num_classes: int = 9,
        attention: bool = True,
        ...
        self._merged_feedback: bool = False
        ...
        if mlp is None:
            self.feedforward = nn.Linear(
                self._pos_lstm.hidden_size + self._neg_lstm.hidden_size + 6, num_classes
            )
        else:
    ...
    def forward(
        ...
        X_pos = torch.nn.utils.rnn.pack_padded_sequence(
            pos_outputs,
            torch.count_nonzero(pos_mask, dim=1).cpu(),
            batch_first=True,
            enforce_sorted=False,
        )
        X_neg = torch.nn.utils.rnn.pack_padded_sequence(
            neg_outputs,
            torch.count_nonzero(neg_mask, dim=1).cpu(),
            batch_first=True,
            enforce_sorted=False,
        )
        ...
        if self._attention:
            pos_dist, pos_outputs = self._pos_attention(
                X_pos, return_attn_distribution=True
            )
            neg_dist, neg_outputs = self._neg_attention(
                X_neg, return_attn_distribution=True
            )
    ...
    def init_pos_hidden(self) -> Tuple[torch.Tensor, torch.Tensor]:
        # the weights are of the form (nb_layers, batch_size, nb_lstm_units)
        hidden_a = torch.randn(
            2 * self._pos_lstm.num_layers, self.batch_size, self._pos_lstm.hidden_size
        )
    ...
    def init_neg_hidden(self) -> Tuple[torch.Tensor, torch.Tensor]:
        # the weights are of the form (nb_layers, batch_size, nb_lstm_units)
        hidden_a = torch.randn(
            2 * self._neg_lstm.num_layers, self.batch_size, self._neg_lstm.hidden_size
        )
        hidden_b = torch.randn(
            2 * self._neg_lstm.num_layers, self.batch_size, self._neg_lstm.hidden_size
        )
...
class ClsMlp(nn.Module):
    ...
    def forward(
        ...
        input_features = torch.cat(
            (pos_outputs[:, 0, :], neg_outputs[:, 0, :], norm_ratings), dim=1
        )  # TODO check dim
        output_features = self.feedforward(input_features)
