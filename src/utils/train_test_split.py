...
TRAIN_PATH_ORIG = "HeadHunter_train.csv"

TRAIN_PATH = "HeadHunter_train_proc.csv"
VAL_PATH = "HeadHunter_val_proc.csv"
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(data_dir, TRAIN_PATH_ORIG)):
        ...
        stratifier = IterativeStratification(
            n_splits=2, order=2, sample_distribution_per_fold=[0.2, 1.0 - 0.2]
        )
        ...
        if os.path.exists(os.path.join(data_dir, TRAIN_PATH_ORIG_NEW)):
            ...
            new_train = train_full.iloc[train_indexes, :]
            new_val = train_full.iloc[val_indexes, :]

            new_train.to_csv(
                os.path.join(data_dir, TRAIN_PATH_NEW), index=False, header=True
            )
            new_val.to_csv(
                os.path.join(data_dir, VAL_PATH_NEW), index=False, header=True
            )
